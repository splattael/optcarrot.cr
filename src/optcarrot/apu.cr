module Optcarrot
  ...
  class APU
    ...
    def clock_frame_irq(target)
      ...
      do_clock_frame_irq
      while @frame_irq_clock <= target
        do_clock_frame_irq
      end
    end

    def do_clock_frame_irq
    ...
    def flush_sound
      if @buffer.size < @settings_rate / 60
        ...
        if @buffer.size < @settings_rate / 60
          clock_frame_counter if @frame_counter < target
          @buffer << @mixer.sample
          while @buffer.size < @settings_rate / 60
            @buffer << @mixer.sample
          end
        end
      end
    ...
    class Oscillator
      ...
      def initialize(apu)
        @apu = apu
        @rate = @fixed = 1
        @envelope = @length_counter = @wave_length = 0
      end
    ...
    class Noise < Oscillator
      ...
      NEXT_BITS_1 = (0..0x7fff).map {|bits| bits.bit(0) == bits.bit(1) ? bits / 2 : bits / 2 + 0x4000 }
      NEXT_BITS_6 = (0..0x7fff).map {|bits| bits.bit(0) == bits.bit(6) ? bits / 2 : bits / 2 + 0x4000 }
