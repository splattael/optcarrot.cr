  ...
  class Config
    OPTIONS = {
      :optimization => {
        :opt_ppu => {
          ...
          # TODO :candidates => PPU::OptimizedCodeBuilder::OPTIONS,
          :default => nil,
        },
        :opt_cpu => {
          :type => :opts,
          :desc => "select CPU optimizations",
          # TODO :candidates => CPU::OptimizedCodeBuilder::OPTIONS,
          :default => nil,
        },
    ...
    OPTIONS.each do |_kind, opts|
      opts.each do |id, opt|
        ...
        # TODO DEFAULT_OPTIONS[id] = opt[:default] if opt.key?(:default)
        # TODO getter id
      end
    end
    getter :romfile
    ...
    class Parser
      ...
      def help
        tbl = ["Usage: #{ $0 } [OPTION]... FILE"]
        long_name_width = 0
        OPTIONS.each do |kind, opts|
          tbl << "" << "#{ kind } options:"
          opts.each do |id_base, opt|
            short_name = opt[:aliases].first
            switch = args = ""
            case opt[:type]
      ...
      def dump_ppu
        # TODO puts PPU::OptimizedCodeBuilder.new(@options[:loglevel], @options[:opt_ppu] || [] of ).build
      end

      def dump_cpu
        # TODO puts CPU::OptimizedCodeBuilder.new(@options[:loglevel], @options[:opt_cpu] || []).build
      end
    end
  end
end
