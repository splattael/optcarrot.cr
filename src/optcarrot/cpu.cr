module Optcarrot
  ...
  class CPU
    ...
    # TODO See https://github.com/crystal-lang/crystal/issues/3613
    CLK_1 = 1 * RP2A03_CC
    CLK_2 = 2 * RP2A03_CC
    CLK_3 = 3 * RP2A03_CC
    CLK_4 = 4 * RP2A03_CC
    ...
    def initialize(conf)
      ...
      @fetch = [nil] * 0x10000
      @store = [nil] * 0x10000
      @peeks = {} of Int32 => Int32
      @pokes = {} of Int32 => Int32
      @ram = [0] * 0x800
    ...
    def run
      do_clock
      # TODO check 2 trailing `while`
      do_opcode
      while @clk < @clk_target
        while @clk < @clk_frame
          do_opcode
        end
        do_opcode
    ...
    # TODO check
    DISPATCH = [] of (Int32 | Symbol | Array(Symbol | Nil | Int32))

    def self.op(opcodes, args)
      opcodes.each do |opcode|
        if args.is_a?(Array) && [:r_op, :w_op, :rw_op].includes?(args[0])
          kind, op, mode = args
          mode = ADDRESSING_MODES[mode][opcode >> 2 & 7]
          send_args = [kind, op, mode]
          send_args << (mode.to_s.starts_with?("zpg") ? :store_zpg : :store_mem) if kind != :r_op
