module Optcarrot
  ...
  class PPU
    ...
    def setup_lut
      @lut_update = Hash(Int32, Array(Int32)).new # TODO was {}.compare_by_identity
      ...
      @name_lut = (0..0xffff).map do |i|
        ...
        # TODO check [] of
        (((@lut_update[nmt_bank] ||= [] of Array(Int32))[nmt_idx] ||= [nil, nil])[0] ||= [] of Array(Int32)) << [i, fixed]
        nmt_bank[nmt_idx] << 4 | fixed
      end

      entries = {} of Array(Int32) => Array(Int32)
      @attr_lut = (0..0x7fff).map do |i|
        ...
        (((@lut_update[nmt_bank] ||= [] of Array(Int32))[nmt_idx] ||= [nil, nil])[1] ||= [] of Array(Int32)) << entries[key]
        entries[key]
      end.freeze
    ...
    def vsync
      ...
      @output_pixels << @palette[15]
      while @output_pixels.size < 256 * 240 # fill black
        @output_pixels << @palette[15]
      end
    end
    ...
    def open_attr
      return unless @any_show
      @io_addr, @bg_pattern_lut_fetched, _ = @attr_lut[@scroll_addr_0_4 + @scroll_addr_5_14]
      update_address_line
    end
    ...
    # TODO removed class OptimizedCodeBuilder
  end
end
